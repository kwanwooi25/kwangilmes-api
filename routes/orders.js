...
const { canReadOrders, canWriteOrders } = require('../middlewares/requirePermission');
...
module.exports = (app) => {
  ...
  GET     /orders-need-plate        전체 주문 조회 (동판필요품목)
	...
	GET     /order-weights/:month     주문 중량 조회 (월별)
		...
		if (!ordersTableExists) {
			db.schema
				...
				.then(() => {
					...
					db.insert({ id: '2018-05', seq: 125 }).into('orders_id_seq').then((result) => {
					...
					db.schema
						...
						.then((result) => {
							...
							db.insert(SAMPLE_ORDERS).into('orders').then((result) => {
  ...
  -----------------------------*/
	app.post('/orders', requireLogin, canReadOrders, (req, res) => {
		...
		date_to = moment(date_to).endOf('day');
		...
		db
			...
			.andWhere('account_name', 'ilike', `%${account_name}%`)
			...
			.catch((error) => res.status(400).json(onRequestFail('error fetching orders')));
	});
  ...
  -----------------------------*/
	app.post('/orders-delivery-schedule', requireLogin, canReadOrders, (req, res) => {
		...
		const date_from = moment([ year, month ]).startOf('month').startOf('day').format('YYYY-MM-DD');
		const date_to = moment([ year, month ]).endOf('month').startOf('day').format('YYYY-MM-DD');
    ...
    전체 주문 조회 (동판필요품목)
  -----------------------------*/
	app.get('/orders-need-plate', requireLogin, canReadOrders, (req, res) => {
		db
			.with('joinedTable', joinedTable)
			.select('*')
			.from('joinedTable')
			.where('is_plate_ready', false)
			.andWhere('is_completed', false)
			.then((orders) => res.json(onRequestSuccess({ orders })))
			.catch((error) => res.status(400).json(onRequestFail('error fetching orders')));
	});
  ...
  -----------------------------*/
    ...
    주문 중량 조회 (월별)
  -----------------------------*/
	app.get('/order-weights/:year/:month', requireLogin, canReadOrders, (req, res) => {
		...
		db
			...
			.then((orders) => {
				...
				// add up order weights
				orders.forEach((order) => {
					let index = orderWeights.findIndex(
						(element) => element.date === moment(order.ordered_at).format('YYYY-MM-DD')
					);

					if (order.is_print === true) {
						orderWeights[index]['is_print'] += order.order_quantity_weight;
					} else if (order.is_print === false) {
						orderWeights[index]['is_not_print'] += order.order_quantity_weight;
					}
				});

				res.json(onRequestSuccess(orderWeights));
			})
			.catch((error) => res.status(400).json(onRequestFail('error fetching orders')));
	});
  ...
  -----------------------------*/
	app.get('/orders/:id', requireLogin, canReadOrders, (req, res) => {
		...
		db
			...
			.catch((error) => res.status(400).json(onRequestFail('error fetching an order')));
	});
  ...
  -----------------------------*/
	app.post('/orders/add', requireLogin, canWriteOrders, (req, res) => {
		...
		const isRequiredEmpty = REQUIRED_PROPS.map((prop) => !!data[prop]).includes(false);
		...
		db('products')
			...
			.then((products) => {
				if (products.length === 0) {
					return res.status(400).json(onRequestFail('존재하지 않는 품목입니다.'));
				} else {
					...
					const getDeliverBy = () => {
						...
						return moment(data.ordered_at).add(leadtime, 'days').format('YYYY-MM-DD');
					};
					...
					// 인쇄일때  && 동판상태 === 확인 = true
					// 무지일때 = true
					data.is_plate_ready = product.is_print && data.plate_status === '확인';
        ...
        --------------------------------------------*/
			...
			.catch((error) => res.status(400).json(onRequestFail('error placing an order')));
	});
  ...
  -----------------------------*/
	app.put('/orders/:id', requireLogin, canWriteOrders, (req, res) => {
		...
		db('orders')
			...
			.then((response) => {
				...
				if (data.plate_status || data.order_quantity) {
					return db('products')
						...
						.where('id', '=', product_id)
						.then((productInfo) => {
							const { product_thick, product_length, product_width, is_print } = productInfo[0];
         ...
         주문정보 수정
        --------------------------------------------*/
				db('orders').where('id', '=', id).update(orderToUpdate).returning('*').then((orders) => {
  ...
  -----------------------------*/
	app.put('/orders-complete', requireLogin, canWriteOrders, (req, res) => {
		...
		Promise.all(
			data.map((order) => {
				return db('orders').where('id', '=', order.id).update(order).returning('*').then((result) => result[0]);
			})
		)
			.then((orders) => res.json(onRequestSuccess(orders)))
	...
	app.delete('/orders', requireLogin, canWriteOrders, (req, res) => {
		...
		if (ids.length) {
			db('orders')
				.whereIn('id', ids)
				.del()
				.then((response) => res.json(onRequestSuccess(`${response}개 주문이 정상적으로 삭제되었습니다.`)))
