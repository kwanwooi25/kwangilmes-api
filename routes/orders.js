...
module.exports = app => {
  ...
  app.post(
    ...
    (req, res) => {
      ...
      const date_from = moment([year, month])
        .startOf('month')
        .startOf('day')
        .format('YYYY-MM-DD');
      const date_to = moment([year, month])
        .endOf('month')
        .startOf('day')
        .format('YYYY-MM-DD');
        ...
        .then(orders => {
          ...
          orders.forEach(order => {
            let index = orderWeights.findIndex(
              element =>
                element.date === moment(order.ordered_at).format('YYYY-MM-DD')
            );

            if (order.is_print === true) {
              orderWeights[index]['is_print'] += order.order_quantity_weight;
            } else if (order.is_print === false) {
              orderWeights[index]['is_not_print'] +=
                order.order_quantity_weight;
            }
          });
