...
const { canReadPlates, canWritePlates } = require('../middlewares/requirePermission');
...
module.exports = (app) => {
  ...
  -----------------------------*/
	app.post('/plates', requireLogin, canReadPlates, (req, res) => {
		...
		db
			...
			.then((data) => {
				Promise.all(
					data.map((plate) => {
						...
						return db('accounts').select('id', 'account_name').whereIn('id', ids).then((response) => {
			...
			.catch((error) => res.status(400).json(onRequestFail('error fetching plates')));
	});
  ...
  -----------------------------*/
	app.post('/plates-for-xls', requireLogin, canReadPlates, (req, res) => {
		const { plate_round = '', plate_length = '', plate_material = '', product_name = '' } = req.body;
  ...
  -----------------------------*/
	app.get('/plates/:id', requireLogin, canReadPlates, (req, res) => {
		...
		db
			...
			.catch((error) => res.status(400).json(onRequestFail('error fetching a plate')));
	});
		...
		if (isRequiredEmpty) {
			...
			Promise.all(
				...
				db
					...
					.catch((error) => res.status(400).json(onRequestFail('error adding plates')));
			});
		}
	});
  ...
  -----------------------------*/
	app.put('/plates/:id', requireLogin, canWritePlates, (req, res) => {
		...
		db('plates')
			...
			.catch((error) => res.status(400).json(onRequestFail('error updating plate')));
	});
	...
	app.delete('/plates', requireLogin, canWritePlates, (req, res) => {
		...
		if (ids.length) {
			db('plates')
				.whereIn('id', ids)
				.del()
				.then((response) => res.json(onRequestSuccess(`${response}개 동판이 정상적으로 삭제되었습니다.`)))
				.catch((error) => res.status(400).json(onRequestFail('error deleting plates')));
		} else {
