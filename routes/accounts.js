...
const { canReadAccounts, canWriteAccounts } = require('../middlewares/requirePermission');
  ...
  -----------------------------*/
	app.post('/accounts', requireLogin, canReadAccounts, (req, res) => {
		...
		db('accounts')
			...
			.catch((error) => res.status(400).json(onRequestFail('error fetching accounts')));
	});
  ...
  -----------------------------*/
	app.get('/accounts/:id', requireLogin, canReadAccounts, (req, res) => {
		...
		db
			...
			.catch((error) => res.status(400).json(onRequestFail('error fetching an account')));
	});
		...
		Promise.all(
			...
			if (!isNameNotEmpty) {
				...
				db
					...
					.returning('*')
					.then((accounts) => res.json(onRequestSuccess(accounts)))
					.catch((error) => res.status(400).json(onRequestFail('error adding accounts')));
			}
		});
	});
	...
	app.put('/accounts/:id', requireLogin, canWriteAccounts, (req, res) => {
		...
		if (!data.account_name) {
			...
			db('accounts')
				...
				.catch((error) => res.status(400).json(onRequestFail('error updating account')));
		}
	});
	...
	app.delete('/accounts', requireLogin, canWriteAccounts, (req, res) => {
		...
		if (ids.length) {
			db('accounts')
				.whereIn('id', ids)
				.del()
				.then((response) => res.json(onRequestSuccess(`${response}개 업체가 정상적으로 삭제되었습니다.`)))
				.catch((error) => res.status(400).json(onRequestFail('error deleting accounts')));
		} else {
